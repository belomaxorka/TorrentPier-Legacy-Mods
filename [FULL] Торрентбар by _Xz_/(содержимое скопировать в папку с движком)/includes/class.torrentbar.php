...
function unlink_dir($dir) //Очистка кеша unlink_dir(BB_ROOT . 'torrentbar/cache/');
{
...
function torrentbar ($user_id, $type = false) //если нужны бары для всех, запускаем функцию с type = all
{
	global $bar;
    ...
    if(!$type)
    {
		return $bar->create($user_id);
	}
	else if($type == 'all')
	{		return $bar->create_all();	}
	else if($type == 'user') //Для обновления уже сохраненного бара  torrentbar ($user_id, 'user');
	{		return $bar->create_user($user_id);	}
}
