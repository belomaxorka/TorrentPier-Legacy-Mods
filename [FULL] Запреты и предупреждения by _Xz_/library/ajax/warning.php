...
if(!empty($this->request['user_id'])) //Для выдачи
{
...
switch($mode)
{
	case 'add':
		...
		if(!IS_ADMIN && ($user_id != $userdata['user_id']))
		{
			...
			{
				$ololo = array('Бан админу - самоубийство',
					'Модер, ты камикадзе?'.
					'Забанишь админа - и кладбище радостно встретит тебя',
					'Не кусай руку с едой!',
					'Модер тебе хана!',
					'И после бана админу ад встретит тебя с радостным воплем "Welcome!@',
					'А теперь сделай ручкой модерству и трекеру',
					'Сцуко! На святое покусился?!',
					'Вот сейчас тебе админ такую баню устроит!',
					'А по печени?',
					'Админ временно недоступен!',
					'Забаненный админ страшнее торнадо',
					'Это ж админ! Кто ж его забанит?!',
					'Покушение на бан админу - от 5 до 15.',
					'Руки прочь от чудотворца!',
					'Не видать тебе модерства, как админу бана',
				);
			...
			{
				$blablabla = array('Бан один на всех модеров сразу!',
					'Поляна общая - не суетись!',
					'Брат на брата?!',
					'Бань модератора! Так его, сцуку!',
					'Глаза разуй - он же модер!',
					'Модер модеру друг, товарищ и брат!',
					'А прав то хватит?)',
					'Низззя!',
					'За наказанием обратитесь к администратору.',
				);
		...
		if($type == 'topic') //Запрет из топика
		{
		...
		else //Запрет из профиля или другого места О_О
		{
		...
		$message = "Уважаемый [b]". $row['username'] ."[/b], администрацией было выявлено нарушение одного из пунктов правил трекера с Вашей стороны![br]
			Рекомендуем Вам ещё раз ознакомиться с [url=". make_url('/viewtopic.php?t=1065') ."]правилами[/url] данного ресурса и не повторять подобных нарушений.[br]
	...
	case 'edit':
		...
		if($auth == DELETED) //Удаление запрета
		{
