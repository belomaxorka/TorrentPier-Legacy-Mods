  ...
  let MagicSuggest = function (element, options) {
    ...
    var self = {
      ...
      _render: function (el) {
        ...
        ms.input.on('paste', handlers._onPaste.bind(this));
    ...
    var handlers = {
      ...
      _onPaste: function (e) {
        ...
        // Разделяем текст по пробелам или запятым
        var items = pastedText.split(/\s*,\s*/).map(function (item) {
          return item.trim(); // Убираем лишние пробелы
        }).filter(function (item) {
          return item.length > 0; // Исключаем пустые строки
        });

        // Преобразуем каждый элемент в объект и добавляем в выборку
        var newItems = items.map(function (item) {
          var obj = {};
          obj[cfg.displayField] = obj[cfg.valueField] = item;
          return obj;
        });

        // Добавляем новые элементы в выборку
        ms.addToSelection(newItems);

        // Очищаем поле ввода
        ms.empty();

        // Если нужно, показываем сообщение о максимальном количестве выбранных элементов
        if (_selection.length === cfg.maxSelection) {
          self._updateHelper(cfg.maxSelectionRenderer.call(this, _selection.length));
        }
      },
