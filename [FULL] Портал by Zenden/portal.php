...
{
	message_die(GENERAL_ERROR, 'Сори, сильно занят, зайдите позже', '', __LINE__, __FILE__, $sql);
}*/
...
if(!bf($userdata['user_opt'], 'user_opt', 'user_portal') ? true : false) //Ленточный интерфейс портала
{
	...
	$topic_count = $row['topic_count'] ? $row['topic_count'] : 0; // Проверка на пустое значение.
	...
	if ($topic_count)
	{
		...
		foreach($row as $rows)
		{
			...
			$template->assign_block_vars('topics', array(
				...
				'POSTER_IMG'		=> $url,
				'PORTAL_POST'		=> $rows['topic_dl_type'] ? true : false,
			));
		}
	}
}
elseif(bf($userdata['user_opt'], 'user_opt', 'user_portal') ? true : false) // Плиточный интерфейс портала
{
	// Начинаем подсчет для паганицы
		...
		for ($j = 0; $j < count($data); $j += 3)
		{
			...
			for ($i = $j; $i < ($j + 3); $i++)
			{
				...
				$template->assign_block_vars('data.topics', array(
					...
					'TOR_ICONS'		=> $bb_cfg['tor_icons'][$data[$i]['tor_status']] ? $bb_cfg['tor_icons'][$data[$i]['tor_status']] : '',
					'TOR_STATUS'	=> $lang['TOR_STATUS_NAME'][$data[$i]['tor_status']] ? $lang['TOR_STATUS_NAME'][$data[$i]['tor_status']] : '',
					...
					'TOPIC_TITLE'	=> wbr(str_short($data[$i]['topic_title'], 40)),
